...
const Auth: React.FC<AuthProps> = ({ mode, onSwitchMode, onBack }) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verificar se é erro de senha vazada (breached password)
      const isBreachedPassword = err.errors?.some((error: any) =>
        error.code === 'form_password_pwned' ||
        error.message?.toLowerCase().includes('data breach') ||
        error.message?.toLowerCase().includes('has been found in')
      );

      if (isBreachedPassword) {
        setBreachedPasswordWarning(true);
        setError(null); // Não mostrar o erro técnico
      } else {
        setBreachedPasswordWarning(false);
            ...
            <div className="animate-in fade-in slide-in-from-left-8 duration-500">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        // Limpar o aviso quando o usuário começar a digitar uma nova senha
                        if (breachedPasswordWarning) {
                          setBreachedPasswordWarning(false);
                        }
                      }}
                      className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition font-medium"
                ...
                {breachedPasswordWarning && (
                  <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex items-start gap-3">
                      <div className="flex-shrink-0 w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center">
                        <ShieldCheck className="text-amber-600" size={20} />
                      </div>
                      <div className="flex-1">
                        <h4 className="font-bold text-amber-900 text-sm mb-1">Escolha uma senha mais forte</h4>
                        <p className="text-amber-800 text-xs leading-relaxed">
                          Ops! Por sua segurança, o sistema bloqueou esta senha.
                          <br /><br />
                          O motivo? Ela foi identificada em listas públicas de vazamentos de outros sites na internet.
                          Isso não significa que você foi hackeado, mas sim que essa combinação é muito comum e insegura.
                          <br /><br />
                          Que tal criar uma senha nova e exclusiva para proteger seus dados aqui no Dentis?
                        </p>
                      </div>
                    </div>
                  </div>
