...
export const Toast: React.FC<{
  ...
  return (
    <motion.div
      ...
      className={`fixed bottom-8 right-8 z-[100] flex items-center gap-3 px-5 py-4 rounded-2xl border shadow-xl ${bgColors[type]} min-w-[300px] bg-white`}
    >
      {icons[type]}
      <p className="text-sm font-bold text-lux-charcoal flex-1">{message}</p>
      {onClose && (
        <button onClick={onClose} className="text-lux-text-secondary hover:text-lux-text transition">
          <X size={16} />
        </button>
      )}
    </motion.div>
